...
import static io.helidon.mcp.tests.declarative.McpResourcesServer.RESOURCE_CONTENT;
import static io.helidon.mcp.tests.declarative.McpResourcesServer.RESOURCE_MEDIA_TYPE;
import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.is;

@ServerTest
class Langchain4jResourcesServerTest {
    private static McpClient client;

    @Service.Inject
    Langchain4jResourcesServerTest(WebServer server) {
        McpTransport transport = new HttpMcpTransport.Builder()
                .sseUrl("http://localhost:" + server.port() + "/resources")
                .logRequests(true)
                .logResponses(true)
                .build();
