...
import static io.helidon.mcp.codegen.McpTypes.MCP_COMPLETION;
import static io.helidon.mcp.codegen.McpTypes.MCP_COMPLETION_CONTENT;
import static io.helidon.mcp.codegen.McpTypes.MCP_COMPLETION_CONTENTS;
import static io.helidon.mcp.codegen.McpTypes.MCP_COMPLETION_INTERFACE;
import static io.helidon.mcp.codegen.McpTypes.MCP_DESCRIPTION;
import static io.helidon.mcp.codegen.McpTypes.MCP_FEATURES;
import static io.helidon.mcp.codegen.McpTypes.MCP_LOGGER;
import static io.helidon.mcp.codegen.McpTypes.MCP_NAME;
import static io.helidon.mcp.codegen.McpTypes.MCP_PARAMETERS;
import static io.helidon.mcp.codegen.McpTypes.MCP_PATH;
import static io.helidon.mcp.codegen.McpTypes.MCP_PROGRESS;
import static io.helidon.mcp.codegen.McpTypes.MCP_PROMPT;
import static io.helidon.mcp.codegen.McpTypes.MCP_PROMPT_CONTENT;
import static io.helidon.mcp.codegen.McpTypes.MCP_PROMPT_CONTENTS;
import static io.helidon.mcp.codegen.McpTypes.MCP_PROMPT_INTERFACE;
import static io.helidon.mcp.codegen.McpTypes.MCP_REQUEST;
import static io.helidon.mcp.codegen.McpTypes.MCP_RESOURCE;
import static io.helidon.mcp.codegen.McpTypes.MCP_RESOURCE_CONTENT;
import static io.helidon.mcp.codegen.McpTypes.MCP_RESOURCE_CONTENTS;
import static io.helidon.mcp.codegen.McpTypes.MCP_RESOURCE_INTERFACE;
import static io.helidon.mcp.codegen.McpTypes.MCP_ROLE;
import static io.helidon.mcp.codegen.McpTypes.MCP_ROLE_ENUM;
import static io.helidon.mcp.codegen.McpTypes.MCP_SERVER;
import static io.helidon.mcp.codegen.McpTypes.MCP_TOOL;
import static io.helidon.mcp.codegen.McpTypes.MCP_TOOL_CONTENT;
import static io.helidon.mcp.codegen.McpTypes.MCP_TOOL_CONTENTS;
import static io.helidon.mcp.codegen.McpTypes.MCP_TOOL_INTERFACE;
import static io.helidon.mcp.codegen.McpTypes.MCP_VERSION;
...
final class McpCodegen implements CodegenExtension {
    ...
    private static final System.Logger LOGGER = System.getLogger(McpCodegen.class.getName());
    ...
    private void process(RoundContext roundCtx, TypeInfo type) {
        ...
        TypeName httpFeature = TypeName.create("io.helidon.webserver.http.HttpFeature");

        var serverClassModel = ClassModel.builder()
                .type(generatedType)
                .addInterface(httpFeature)
        ...
        serverClassModel.addImport("io.helidon.mcp.server.McpServerConfig");
    ...
    private void addRoutingMethod(Method.Builder method, TypeInfo type) {
        TypeName routing = TypeName.create("io.helidon.webserver.http.HttpRouting.Builder");

        method.name("setup")
                .accessModifier(AccessModifier.PUBLIC)
                .returnType(TypeNames.PRIMITIVE_VOID)
                .addAnnotation(Annotations.OVERRIDE)
                .addParameter(rules -> rules.type(routing).name("routing"));

        method.addContentLine("McpServerConfig.Builder builder = McpServerConfig.builder();");
    ...
    private void addCompletionMethod(Method.Builder builder, ClassModel.Builder classModel, TypedElementInfo element) {
        ...
        builder.name("completion")
                .returnType(returned -> returned.type("Function<McpRequest, " + MCP_COMPLETION_CONTENT + ">"))
                .addAnnotation(Annotations.OVERRIDE);
        ...
        for (TypedElementInfo param : element.parameterArguments()) {
            ...
            if ("java.lang.String".equals(param.typeName().fqName())) {
    ...
    private void generatePrompts(ClassModel.Builder classModel, TypeInfo type) {
        ...
        classModel.addImport(Set.class)
                .addImport(TypeName.create("io.helidon.mcp.server.McpPromptArgument"));
            ...
            if (prompts.contains(className)) {
                if (LOGGER.isLoggable(System.Logger.Level.WARNING)) {
                    LOGGER.log(System.Logger.Level.WARNING,
                               "Prompt '%s' already exists. Use @Mcp.Name or change the method name.", element.elementName());
                }
            }
    ...
    private void generateTools(ClassModel.Builder classModel, TypeInfo type) {
        ...
        for (TypedElementInfo element : elements) {
            ...
            String description = element.annotation(MCP_TOOL).value().orElse("");
    ...
    private void addToolSchemaMethod(Method.Builder builder, TypedElementInfo element) {
        ...
        for (TypedElementInfo param : element.parameterArguments()) {
            ...
            fields.add(TypedElementInfo.builder()
        ...
        if (!fields.isEmpty()) {
            McpJsonSchemaCodegen.addSchemaMethodBody(method, fields);
        } else {
