...
import static io.helidon.mcp.codegen.McpTypes.MCP_JSON_SCHEMA;
import static io.helidon.service.codegen.ServiceCodegenTypes.SERVICE_ANNOTATION_SINGLETON;

/**
 * For each class annotated with {@link io.helidon.mcp.server.Mcp.JsonSchema},
...
class McpJsonSchemaCodegen implements CodegenExtension {
    ...
    private static final System.Logger LOGGER = System.getLogger(McpJsonSchemaCodegen.class.getName());

    static final String PROPERTY_SCHEMA = """
            builder.append("\\"%s\\": { \\"type\\": \\"%s\\" }");
            """;
    ...
    private void process(RoundContext roundContext, TypeInfo type) {
        ...
        List<TypedElementInfo> fields = type.elementInfo()
                .stream()
    ...
    static void addSchemaMethodBody(Method.Builder method, List<TypedElementInfo> fields) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            addPropertySchema(method, field, i == n - 1);
        }
    ...
    static void addPropertySchema(Method.Builder method, TypedElementInfo element, boolean last) {
        TypeName typeName = element.typeName();
        if (TypeNames.STRING.equals(typeName) || typeName.primitive()) {
            method.addContent(String.format(PROPERTY_SCHEMA,
                                            element.elementName(),
